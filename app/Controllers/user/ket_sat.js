      ...
      if (user) {
        if (user.red < red) {
          ...
          LScuoc.create({
            uid: client.UID,
            lswin: -red,
            chitiet: "Gửi Tiền Vào Két",
            thanhtoan: 0,
            tienhienco: user.red - red,
            dichvu: "Két Sắt",
            time: new Date(),
          });
        }
    ...
    UserInfo.findOne({ id: client.UID }, "red ketSat", function (err, user) {
      if (user.ketSat < red) {
        ...
        client.red({
          ...
          user: { red: user.red * 1 + red, ketSat: user.ketSat - red },
        });

        LScuoc.create({
          uid: client.UID,
          lswin: red,
          chitiet: "Rút Tiền trong Két",
          thanhtoan: 0,
          tienhienco: user.red * 1 + red,
          dichvu: "Két Sắt",
          time: new Date(),
        });
      }
