...
function sendOTP(client, data) {
  ...
  if (!!mavung) {
    switch (mavung) {
      ...
      case "0":
        client.red({
          notice: { title: "LỖI", text: "Vui lòng chọn lại mã vùng !" },
        });
        break;
    }
  } else {
  ...
  if (!!phone && helper.checkPhoneValid(phone)) {
    ...
    if (phoneCrack) {
            ...
            Phone.findOne({ phone: phone }, function (err1, crack) {
              if (crack) {
                client.red({
                  notice: {
                    title: "LỖI",
                    text: "Số điện thoại đã tồn tại trên hệ thống 1.!",
                  },
                });
              } else {
                var otp = (Math.random() * (9999 - 1000 + 1) + 1000) >> 0; // từ 1000 đến 9999
                OTP.findOne(
                  { uid: client.UID, phone: phone },
                  {},
                  { sort: { _id: -1 } },
                  function (err2, data) {
                    if (
                            ...
                            if (type == "1") {
                                ...
                                function (err3, teleCheck) {
                                  if (teleCheck) {
                                    client.red({
                                      notice: {
                                        title: "OTP",
                                        text: "Vui lòng mở ứng dụng telegram và kiểm tra tin nhắn.!",
                                      },
                                    });
                                      ...
                                      function (err4, teleCheck1) {
                                        if (teleCheck1) {
                                          ...
                                          client.red({
                                            notice: {
                                              title: "OTP",
                                              text: "Vui lòng mở ứng dụng telegram và chia chia sẻ số điện thoại rồi lấy OTP.!",
                                            },
                                          });
                                        }
                                      }
                              ...
                              if (user.red < 2000) {
                                ...
                                client.red({
                                  notice: {
                                    title: "THÔNG BÁO",
                                    text: "Mã OTP đã được gửi tới số điện thoại của bạn.",
                                  },
                                });
                              }
                            }
                      ...
                      client.red({
                        notice: {
                          title: "OTP",
                          text: "Vui lòng mở điện thoại và kiểm tra tin nhắn.!",
                        },
                      });
                    }
                  }
      ...
      client.red({
        notice: { title: "THÔNG BÁO", text: "Số điện thoại không hợp lệ.!" },
      });
    }
  } else {
    client.red({
            ...
            Phone.findOne({ phone: phone }, function (err1, crack) {
              if (crack) {
                client.red({
                  notice: {
                    title: "LỖI",
                    text: "Số điện thoại đã tồn tại trên hệ thống 2.!",
                  },
                });
              } else {
                var otp = (Math.random() * (9999 - 1000 + 1) + 1000) >> 0; // từ 1000 đến 9999
...
function regOTP(client, data) {
  ...
  if (!!data && !!data.phone && !!data.otp) {
    if (!helper.checkPhoneValid(phone)) {
      client.red({
        notice: { title: "LỖI", text: "Số điện thoại không hợp lệ" },
      });
    } else if (!validator.isLength(data.otp, { min: 4, max: 6 }) && false) {
      //ignore otp
      ...
      if (phoneCrack) {
        OTP.findOne(
          { uid: client.UID, phone: phoneCrack },
          {},
          { sort: { _id: -1 } },
          function (err1, data_otp) {
            if (data_otp && data.otp == data_otp.code) {
              if (
                ...
                client.red({
                  notice: { title: "LỖI", text: "Mã OTP đã hết hạn.!" },
                });
              } else {
                // Xác thực thành công
                ...
                client.red({
                  ...
                  user: { phone: phoneCrack, email: "", cmt: "" },
                });
              }
            } else {
              client.red({
                notice: { title: "LỖI", text: "Mã OTP Không đúng.!" },
              });
            }
          }
        );
