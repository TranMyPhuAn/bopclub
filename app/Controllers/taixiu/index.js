...
function getLogs(client) {
      ...
      function (err, post) {
        Promise.all(
          post.map(function (obj) {
            return { dice: [obj.dice1, obj.dice2, obj.dice3], phien: obj.id };
          })
  ...
  var active2 = new Promise((resolve, reject) => {
    TaiXiu_User.findOne(
      { uid: client.UID },
      "tLineWinRed tLineLostRed tLineWinRedH tLineLostRedH",
      function (err, data_a2) {
          ...
          TXChat.findOne(
            ...
            function (err, post) {
              if (
                !post ||
                post.uid != client.UID ||
                (post.uid == client.UID && post.value != str)
              ) {
                TXChat.create({
                  uid: client.UID,
                ...
                var content = {
                  taixiu: {
                    chat: {
                      message: { user: client.profile.name, value: str },
                    },
                  },
                };
                Promise.all(
                  Object.values(client.redT.users).map(function (users) {
                    // console.log("client.redT.users", client.redT.user);
                    Promise.all(
...
var cuoc = function (client, data) {
  if (!!data && !!data.bet) {
    if (client.redT.TaiXiu_time < 3 || client.redT.TaiXiu_time > 64) {
      // đang 60s mà em chua tính trả thưởng
          ...
          if (user === null || user.red < bet) {
            ...
            DaiLy.findOne({ nickname: user.name }, function (err, userDl) {
              if (userDl) {
                ...
                TXCuocOne.findOne(
                  { uid: client.UID, phien: phien },
                  "bet phien select",
                  function (isCuocErr, isCuoc) {
                    if (!!isCuoc) {
                      // update
                      if (isCuoc.select !== select) {
                        client.red({
                          taixiu: { err: "Chỉ được cược 1 bên.!!" },
                        });
                      } else {
                        user.red -= bet;
                        user.save();
                        ...
                        LScuoc.updateOne(
                          { uid: client.UID, phien: phien },
                          { $set: { tienhienco: user.red }, $inc: { bet: bet } }
                        ).exec();
                        ...
                        var taixiuVery = select
                          ? { red_me_tai: isCuoc.bet }
                          : { red_me_xiu: isCuoc.bet };
    ...
    TXCuoc.find(
      ...
      function (err, list) {
        if (list.length) {
          ...
          let TaiXiu_tong_red_lech = Math.abs(
            TaiXiu_red_tong_tai - TaiXiu_red_tong_xiu
          );
          let TaiXiu_red_lech_tai =
  ...
  if (!!data && !!data.phien) {
    ...
    Promise.all([getPhien, getCuoc]).then((values) => {
      if (!!values[0]) {
        ...
        client.red({
          notice: { title: "LỖI", text: "Phiên không tồn tại...", load: false },
        });
        phien = null;
        getPhien = null;
        //getCuoc  = null;
  ...
  if (phienis == 0) {
    HUTX.findOne(
      ...
      function (err, last) {
        ...
        HUTX.find(
          { phiennohu: getphienhu },
          {},
          { sort: { nhan: -1 }, limit: 39 },
              ...
              results.map(function (obj) {
                return new Promise(function (resolve, reject) {
                  UserInfo.findOne({ id: obj.uid }, function (error, result2) {
                    resolve({
                      name: !!result2 ? result2.name : "",
                      phiennohu: obj.phiennohu,
                      bet: obj.nhan,
                      phien: obj.phien,
                      quyhu: obj.quyhu,
                      ketqua: obj.ketqua,
                      tonguser: obj.tonguser,
                      time: obj.time,
                    });
                  });
                });
        ...
        var getCuoc = LScuoc.find(
          ...
          function (error, result) {
            if (result.length) {
              ...
              client.red({
                taixiu: {
                  get_lichsuchoi: { data: [], page: page, kmess: 6, total: 0 },
                },
              });
              page = null;
              client = null;
...
var get_log = function (client, data) {
  if (!!data && !!data.page) {
    ...
    var kmess = 9;
    if (page > 0) {
      TXCuoc.countDocuments({ uid: client.UID, thanhtoan: true }).exec(
        function (err, total) {
          var getCuoc = TXCuoc.find(
            { uid: client.UID, thanhtoan: true },
            {},
            { sort: { _id: -1 }, skip: (page - 1) * kmess, limit: kmess },
            function (error, result) {
              if (result.length) {
                ...
                client.red({
                  taixiu: {
                    get_log: { data: [], page: page, kmess: 9, total: 0 },
                  },
                });
                page = null;
                client = null;
