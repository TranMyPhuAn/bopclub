...
let signName = function (client, name) {
  if (!!name) {
    ...
    if (!validator.isLength(name, { min: 3, max: 14 })) {
      client.red({
        notice: { title: "TÊN NHÂN VẬT", text: "Độ dài từ 3 đến 14 ký tự !!" },
      });
    } else if (!testName) {
      client.red({
        notice: {
          title: "TÊN NHÂN VẬT",
          text: "Tên không chứa ký tự đặc biệt !!",
        },
      });
    } else {
      UserInfo.findOne(
            ...
            User.findOne({ _id: client.UID }, function (err, base) {
              ...
              if (testBase) {
                client.red({
                  notice: {
                    title: "TÊN NHÂN VẬT",
                    text: "Tên nhân vật không được trùng với tên đăng nhập...",
                  },
                });
              } else {
                      ...
                      UserInfo.create(
                        ...
                        function (errC, user) {
                          if (!!errC) {
                            ...
                            let text = `Cổng game uy tín xanh chín nhất BOPCLUB
...
let changePassword = function (client, data) {
  if (!!data && !!data.passOld && !!data.passNew && !!data.passNew2) {
    if (!validator.isLength(data.passOld, { min: 6, max: 32 })) {
      client.red({
        notice: { title: "LỖI", text: "Độ dài mật khẩu từ 6 đến 32 ký tự !!" },
      });
      ...
      client.red({
        notice: { title: "LỖI", text: "Nhập lại mật khẩu không đúng.!!" },
      });
    } else {
        ...
        if (!!user) {
          if (user.local.username == data.passNew) {
            client.red({
              notice: {
                title: "LỖI",
                text: "Mật khẩu không được trùng với tên đăng nhập.!!",
              },
            });
          } else {
