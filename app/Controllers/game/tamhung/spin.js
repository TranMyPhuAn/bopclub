  ...
  if (!!data && !!data.cuoc && Array.isArray(data.line)) {
    ...
    if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
      ...
      if (client.tamhung.free > 0 && bet != client.tamhung.lastBet) {
        client.red({
          tamhung: { notice: "Bạn không nên đổi cược khi đang free spin" },
        });
      } else {
        ...
        UserInfo.findOne(
          { id: client.UID },
          red ? "red name daily" : "xu namedaily",
          function (err, user) {
            if (
              client.tamhung.free === 0 &&
              ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))
            ) {
              client.red({
                tamhung: { status: 0, notice: "Bạn không đủ số dư" },
              });
            } else {
              if (user.daily != "")
                Helpers.pushDailyVIP({
                  daily: user.daily,
                  reason:
                    "Cộng tiền người chơi " + user.name + " chơi TAM HÙNG",
                  type: true,
                  total: tongCuoc,
                });
                    ...
                    if (red) {
                      ...
                      client.red({
                        tamhung: {
                          ...
                          line_win: result,
                          win: bet_win,
                          free: client.tamhung.free,
                          isFree: isFree,
                          isBonus: !!client.tamhung.bonusX,
                          isNoHu: nohu,
                          isBigWin: isBigWin,
                        },
