...
module.exports = function (client, data) {
  if (!!data && !!data.sotien && !!data.captcha) {
    ...
    if (!validator.isLength(data.captcha, { min: 4, max: 4 })) {
      client.red({
        notice: { title: "", text: "Captcha không đúng!", load: false },
      });
    } else if (validator.isEmpty(data.sotien)) {
      client.red({
        notice: { title: "", text: "Vui lòng nhập số tiền nạp!", load: false },
      });
    } else if (money < MomoBonus.min) {
      client.red({
        notice: {
          title: "LỖI",
          text: `Nạp tối thiểu ${helper.numberWithCommas(
            MomoBonus.min
          )}, tối đa ${helper.numberWithCommas(MomoBonus.max)}`,
          load: false,
        },
      });
    } else {
      ...
      if (checkCaptcha) {
        try {
          Bank_history.findOne(
            { uid: client.UID },
            "id status type info name number bank money",
            { sort: { id: -1 } },
            function (err, last) {
              if (!!last) {
                    ...
                    function (err, check) {
                      ...
                      client.red({
                        notice: {
                          ...
                          text:
                            "Bạn đang có Giao dịch nạp chờ xử lý, Vui lòng chuyển khoản vào *" +
                            last.bank +
                            "* *" +
                            last.number +
                            "* với số tiền *" +
                            ...
                            "* nội dung *" +
                            last.info +
                            "* hoặc vào lịch sử để hủy Giao dịch trước đó !  ",
                          load: false,
                        },
                      });
                    }
                    ...
                    function (err, checkacc) {
                      UserInfo.findOne(
                        ...
                        function (err, check) {
                          ...
                          client.red({
                            notice: {
                              title: "Lệnh nạp được khởi tạo thành công",
                              text: `Vui lòng chuyển tiền tới \n` + nap.phone,
                              load: false,
                            },
                          });
                        }
