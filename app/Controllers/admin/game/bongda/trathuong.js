  ...
  if (!!data && !!data.win && !!data.phien) {
    ...
    if (Helper.isEmpty(select) || Helper.isEmpty(phien)) {
      client.red({
        notice: {
          title: "VIPRIK BET",
          text: "Không bỏ trống các thông tin...",
        },
      });
    } else {
      tabBongDa.findOne({ phien: phien }, function (err, info) {
        if (!!info) {
          if (info.status) {
            client.red({
              notice: { title: "VIPRIK BET", text: "Phiên đã trả thưởng..." },
            });
          } else {
            BongDa_cuoc.find({ phien: phien }, {}, function (errC, cuoc) {
              if (cuoc.length > 0) {
                client.red({
                  notice: { title: "VIPRIK BET", text: "Success" },
                });
                let tongCuoc = 0;
                let tongTra = 0;
                info.status = true;
          ...
          client.red({
            notice: { title: "VIPRIK BET", text: "Trả thưởng thất bại..." },
          });
        }
      });
    }
