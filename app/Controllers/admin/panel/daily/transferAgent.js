    ...
    if (!validator.isLength(data.name, { min: 3, max: 17 })) {
      ...
      if (
        ...
        Phone.findOne({ uid: userAuth.id }, {}, function (err, check) {
          if (check) {
            OTP.findOne(
              { uid: userAuth.id, phone: check.phone },
              {},
              { sort: { _id: -1 } },
              function (err, data_otp) {
                if (data_otp && otp == data_otp.code) {
                  if (
                    ...
                    var active2 = UserInfo.findOne(
                      { name: name },
                      "id name red"
                    ).exec();
                    var active3 = UserInfo.findOne(
                      { id: userAuth.id },
                      "red"
                    ).exec();
                    Promise.all([active1, active2, active3]).then(
                      (valuesCheck) => {
                        ...
                        if (!!to) {
                          if (to.id == userAuth.id) {
                            res.json({
                              ...
                              data: {
                                nickname:
                                  "Bạn không thể chuyển cho chính mình.",
                              },
                            });
                          } else {
                            if (user == null || user.red - 10000 < red) {
                              ...
                              if (
                                void 0 !== data.message &&
                                !validator.isEmpty(data.message.trim())
                              ) {
                                ...
                                function (err, result) {
                                  if (!!result) {
                                    if (void 0 !== redT.users[to.id]) {
                                      Promise.all(
                                        redT.users[to.id].map(function (obj) {
                                          obj.red({
                                            notice: {
                                              title: "CHUYỂN R",
                                              text:
