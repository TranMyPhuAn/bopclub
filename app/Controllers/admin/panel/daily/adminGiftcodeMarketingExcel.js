...
module.exports = function (req, res) {
  ...
  const createSheet = () => {
    return new Promise((resolve) => {
      ...
      ws.cell(1, 1).string("STT").style(styleHeader);

      ws.cell(1, 2).string("Mã code").style(styleHeader);

      ws.cell(1, 3).string("Mệnh giá").style(styleHeader);

      ws.cell(1, 4).string("Ngày xuất").style(styleHeader);

      ws.cell(1, 5).string("Ngày hết hạn").style(styleHeader);

      ws.cell(1, 6).string("Tình trạng").style(styleHeader);
      ...
      GiftCode.find(filter, {}, { sort: { _id: -1 } }, function (err, result) {
        ...
        result.map(function (item, index) {
          ...
          ws.cell(row, 2).string(item.code).style(styleColumn);
