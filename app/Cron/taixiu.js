...
let botchat = function () {
  botChat = setInterval(function () {
    ...
    if (botListChat.length > 0) {
      TXBotChat.aggregate([{ $sample: { size: 1 } }]).exec(function (
        err,
        chatText
      ) {
        let dataBot = {};
...
let TopHu = function () {
  HU_game.find({}, "game type hutx red bet toX balans x").exec(function (
    err,
    data
  ) {
    if (data.length) {
            ...
            TaiXiu_User.findOne({ uid: obj.uid }, function (error, data) {
              if (!!data) {
                ...
                if (bet_thua >= 10000) {
                  update = {
                    tLineWinRed:
                      obj.win && data.tLineWinRed < data.tLineWinRedH + 1
                        ? data.tLineWinRedH + 1
                        : data.tLineWinRed,
                    tLineLostRed:
                      !obj.win && data.tLineLostRed < data.tLineLostRedH + 1
                        ? data.tLineLostRedH + 1
                        : data.tLineLostRed,
                ...
                if (void 0 !== io.users[obj.uid] && obj.win > 0) {
                  io.users[obj.uid].forEach(function (client) {
                    client.red({
                      taixiu: {
                        status: { win: obj.win, select: obj.select, bet: bet },
                      },
                    });
                  });
                }
...
let thongtin_thanhtoan = function (game_id, dice = false) {
  if (dice) {
    ...
    let home = {
      taixiu: {
        taixiu: {
          red_tai: io.taixiu.taixiu.red_tai,
          red_xiu: io.taixiu.taixiu.red_xiu,
        },
      },
    };
